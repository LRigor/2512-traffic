...
export default function AICardDetail({ tool, allTools = [] }: AICardDetailProps) {
  ...
  const useCases = [
    { role: "Project Managers", description: "Organizing and scheduling tasks for team members." },
    { role: "Content Creators", description: "Generating high-quality content ideas and drafts." },
    { role: "Data Analysts", description: "Analyzing complex datasets to retrieve actionable insights." },
    { role: "Customer Support Teams", description: "Automating responses to common customer inquiries." },
    { role: "Marketing Teams", description: "Creating targeted marketing campaigns and analyzing their effectiveness." },
    { role: "Freelancers", description: "Managing multiple projects and deadlines efficiently." },
    { role: "HR Professionals", description: "Screening and managing job applications." },
    { role: "Small Business Owners", description: "Automating everyday business tasks to save time." },
  ];
      ...
      <div className="container mx-auto px-4 py-4 max-w-7xl">
        ...
        <div className="mb-8">
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              <h1 className="text-4xl font-bold text-black mb-4">{tool.tool_name}</h1>
              <div className="flex items-center gap-4 mb-4">
                <div className="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-md">
                  <Image src="/up-arrow-blank.svg" alt="Upvote" width={16} height={16} />
                  <span className="font-medium text-gray-700">{tool.favouriteCount}</span>
                </div>
                <div className="bg-red-100 border border-red-300 text-red-800 px-3 py-1 rounded-md text-sm font-medium flex items-center gap-2">
                  <Image src="/up-arrow-blank.svg" alt="Upvote" width={14} height={14} />
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex items-center">
                  {Array.from({ length: fullStars }).map((_, i) => (
                    <Image key={i} src="/star.svg" alt="Full star" width={20} height={20} className="h-5 w-5" />
                  ))}
                  {hasHalfStar && (
                    <Image src="/star-half.svg" alt="Half star" width={20} height={20} className="h-5 w-5" />
                  )}
                </div>
                <span className="text-sm text-gray-600 ml-2">{tool.review_count} reviews</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <section className="mb-12">
          ...
          <p className="text-gray-600 text-sm mb-6">
            {tool.tool_name} is a next generation AI assistant built for work and trained to be safe, accurate, and secure. BY ANTHROPIC
          </p>
          <div className="mb-6">
        ...
        {/* Top Features */}
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-black mb-6">{tool.tool_name}&apos;s Top Features</h2>
        ...
        {/* Pricing Section */}
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-black mb-4">{tool.tool_name}&apos;s pricing</h2>
        ...
        <section className="mb-12">
          ...
          <div className="bg-gray-50 p-6 rounded-lg mb-8">
            ...
            <div className="flex gap-1 mb-4">
              {[1, 2, 3, 4, 5].map((star) => (
                <Image key={star} src="/star-empty.svg" alt="Star" width={24} height={24} className="h-6 w-6" />
              ))}
            </div>
            <textarea
          ...
          <div className="space-y-4">
            {reviews.map((review, index) => (
              <div key={index} className="flex items-start gap-4 pb-4 border-b border-gray-200">
                <div
            ...
            {useCases.map((useCase, index) => (
              <div key={index} className="border-b border-gray-200">
                <button
                  ...
                  className="w-full flex items-center justify-between py-4 text-left hover:bg-gray-50 transition-colors"
                >
                  <span className="font-semibold text-blue-600">{useCase.role}</span>
                  <svg
                    className={`w-5 h-5 text-gray-500 transition-transform ${
        ...
        {similarToolsData.length > 0 && (
          <section className="mb-12">
            <h2 className="text-2xl font-bold text-black mb-6">Top {tool.tool_name} Alternatives</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {similarToolsData.map((similarTool) => (
                <div key={similarTool.id} className="border rounded-lg p-4 hover:shadow-lg transition-shadow">
                  <Image
                  ...
                  />
                  <h3 className="font-semibold text-lg mb-2">{similarTool.tool_name}</h3>
                  <div className="flex items-center gap-2 mb-2">
                    <Image src="/up-arrow-blank.svg" alt="Upvote" width={16} height={16} />
                    <span className="text-sm text-gray-600">{similarTool.favouriteCount}</span>
                  </div>
                  <p className="text-sm text-gray-600 mb-4 line-clamp-2">{similarTool.headline}</p>
